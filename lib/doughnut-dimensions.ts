export type DoughnutDomain = "social_foundation" | "ecological_ceiling";

export interface DoughnutDimension {
  key: string;
  name: string;
  domain: DoughnutDomain;
}

export const DOUGHNUT_DIMENSIONS: DoughnutDimension[] = [
...
export const DIMENSION_BY_KEY = new Map(DOUGHNUT_DIMENSIONS.map((dimension) => [dimension.key, dimension]));
export const DIMENSION_KEY_SET = new Set(DOUGHNUT_DIMENSIONS.map((dimension) => dimension.key));
